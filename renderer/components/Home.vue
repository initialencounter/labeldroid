...
import { Config } from 'labeldroid-rs';
...
async function getConfig() {
  try {
    const config: Config = await apiManager.get('/api/config');
...
const updateShortcuts = async (config: Config) => {
  if (config.shortcuts) {
    const shortcuts = config.shortcuts;
    registerShortcuts(
      () => {
        sidebarRef.value?.jumpToPrev();
      },
      shortcuts.open_prev,
      'open_prev',
    );
...
const registerShortcuts = async (
  ...
  if (shortcuts) {
    console.log('Registering shortcut:', shortcutname, shortcuts);
